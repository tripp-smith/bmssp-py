...
fn sssp_csr_impl<T>(
    ...
    weights: PyReadonlyArray1<T>,
    source: usize,
    enabled: Option<PyReadonlyArray1<u8>>,
    return_pred: bool,
) -> PyResult<PyObject>
where
    T: numpy::Element + Copy + pyo3::types::PyFloatLike + 'static,
    f32: From<T>,
    f64: From<T>,
{
    ...
    // Convert weights to f32/f64 for algorithm
    // This is a limitation - we'd need generic algorithm implementation
    // For now, convert to f64 for both
    let weights_f64: Vec<f64> = weights_slice.iter().map(|&w| f64::from(w)).collect();
    
    // Run Dijkstra with predecessors if requested
    let (dist_f64, pred_vec) = dijkstra_sssp_with_preds(
        &graph,
        &weights_f64,
        source,
    ...
    let dist: Vec<T> = dist_f64.iter().map(|&d| T::from_f64(d).unwrap_or(T::zero())).collect();
...
pub fn sssp_f32_csr(
    ...
    sssp_csr_impl(py, indptr, indices, weights, source, enabled, return_pred)
}
